...
namespace LiteDB
{
    public class BsonDocument : BsonValue
    {
        ...
        public BsonValue this[string name]
        {
            ...
            {
                BsonValue value;
                if (this.RawValue.TryGetValue(name, out value))
                {
                    return value;
                }
                else
        ...
        public IEnumerable<string> Keys
        {
            get
            {
                ...
                if (this.RawValue.ContainsKey("_id"))
                {
                    //return id first
                    yield return "_id";
                }
                foreach (var key in this.RawValue.Keys)
                {
                    if (key != "_id")
                    {
                        yield return key;
        ...
        public BsonValue Get(string path)
        {
            ...
            return value[names[names.Length - 1]];
        }
        ...
        public BsonDocument Set(string path, BsonValue value)
        {
            ...
            doc[names[names.Length - 1]] = value;
